...
typedef struct {
    ...
    const char* confirmed_status;  // text displayed in confirmation page (after
                                   // long press)
    const char* rejected_status;   // text displayed in rejection page (after
                                   // reject confirmed)
...
enum {
    ...
    BACK_TOKEN_SELFTRANSFER,  // special case when it's a self-transfer (no
                              // external outputs)
...
static void status_confirmation_callback(bool confirm) {
    if (confirm) {
        ...
        ux_flow_response_false();
        nbgl_useCaseStatus(transactionContext.rejected_status, false,
                           ui_menu_main);
    }
}

static void status_cancel(void) { status_confirmation_callback(false); }

static void confirm_cancel(void) {
    nbgl_useCaseConfirm("Reject transaction?", "", "Yes, Reject",
                        "Go back to transaction", status_cancel);
}
...
static void sign_message_callback(void) {
    ...
    nbgl_useCaseStaticReview(&transactionContext.tagValueList,
                             &transactionContext.infoLongPress, "Reject",
                             start_processing_callback);
}
...
static void transaction_confirm(int token, uint8_t index) {
    ...
    int backToken = transactionContext.extOutputCount == 0
                        ? BACK_TOKEN_SELFTRANSFER
                        : BACK_TOKEN_TRANSACTION;
    if (transactionContext.signSender)
        backToken = transactionContext.extOutputCount == 0
                        ? BACK_TOKEN_SELFTRANSFER_SENDER
                        : BACK_TOKEN_TRANSACTION_SENDER;

    if (token == CONFIRM_TOKEN) {
        nbgl_pageNavigationInfo_t info = {
            .activePage = transactionContext.extOutputCount + 1,
        ...
        nbgl_pageDrawGenericContent(&transaction_confirm_callback, &info,
                                    &content);
        nbgl_refresh();
    } else {
        confirm_cancel();
...
void ui_accept_transaction_flow(bool is_self_transfer, bool sign_sender) {
    ...
    if (sign_sender) {
        ...
        transactionContext.tagValuePair[0].value =
            g_ui_state.validate_transaction.fee;
    ...
    transactionContext.confirm = sign_sender
                                     ? "Sign sender\nfor contract?"
                                     : "Sign transaction\nto send Qtum?";
    ...
    nbgl_pageNavigationInfo_t info = {
        .activePage = transactionContext.extOutputCount,
        .nbPages = transactionContext.extOutputCount + 2,
        .navType = NAV_WITH_TAP,
        .progressIndicator = true,
...
void ui_display_transaction_prompt(const int external_outputs_total_count,
    ...
    nbgl_useCaseReviewStart(&ICON_APP,
                            sign_sender ? "Review contract\ntransaction"
                                        : "Review transaction\nto send Qtum",
                            "", "Reject transaction", ux_flow_response_true,
                            confirm_cancel);
}
...
static void display_output(void) {
    ...
    nbgl_pageNavigationInfo_t info = {
        .activePage = transactionContext.currentOutput - 1,
        .nbPages = transactionContext.extOutputCount + 2,
        .navType = NAV_WITH_TAP,
        .progressIndicator = true,
        .navWithTap.backButton = false,
...
void ui_display_output_address_amount_flow(int index) {
    ...
    transactionContext.tagValuePair[1].value =
        g_ui_state.validate_output.amount;

    size_t out_len =
        MAX(MAX_ADDRESS_LENGTH_STR + 1, MAX_OPRETURN_OUTPUT_DESC_SIZE_SHORT);
    bool hasDelegate =
        get_delegate_data(g_ui_state.validate_output.address_or_description,
                          out_len, g_ui_state.validate_output.staker_fee);
    if (hasDelegate) {
        ...
        transactionContext.tagValuePair[3].value =
...
void ui_display_pubkey_flow(void) {
    ...
    transactionContext.tagValuePair[0].value =
        g_ui_state.path_and_pubkey.bip32_path_str;

    transactionContext.tagValuePair[1].item = "Public key";
    transactionContext.tagValuePair[1].value =
        g_ui_state.path_and_pubkey.pubkey;
    transactionContext.tagValueList.nbPairs = 2;
    ...
    nbgl_useCaseReviewStart(&ICON_APP, "Confirm public key", "", "Cancel",
                            continue_light_notify_callback, status_cancel);
}
...
void ui_display_receive_in_wallet_flow(void) {
    ...
    nbgl_useCaseReviewStart(&ICON_APP, "Receive\nin known wallet", "", "Cancel",
                            continue_light_notify_callback, status_cancel);
}
...
void ui_display_policy_map_cosigner_pubkey_flow(void) {
    ...
    transactionContext.tagValuePair[0].value =
        g_ui_state.cosigner_pubkey_and_index.signer_index;

    transactionContext.tagValuePair[1].item = "Public key";
    transactionContext.tagValuePair[1].value =
        g_ui_state.cosigner_pubkey_and_index.pubkey;
    ...
    nbgl_useCaseReviewStart(&ICON_APP, "Register cosigner", "", "Cancel",
...
static void suspicious_pubkey_warning(void) {
    nbgl_useCaseReviewStart(
        &IMPORTANT_CIRCLE_ICON, "WARNING", "The derivation path\nis unusual",
        "Cancel", continue_light_notify_callback, ux_flow_response_false);
}
...
void ui_display_pubkey_suspicious_flow(void) {
    ...
    transactionContext.rejected_status = "Public key rejected";
    nbgl_useCaseReviewStart(&ICON_APP, "Confirm public key", "", "Cancel",
                            suspicious_pubkey_warning, status_cancel);
}
...
void ui_display_register_wallet_flow(void) {
    ...
    transactionContext.tagValuePair[1].value =
        g_ui_state.wallet.descriptor_template;
    ...
    nbgl_useCaseReviewStart(&ICON_APP, "Register wallet", "", "Cancel",
...
void ui_sign_message_flow(void) {
    transactionContext.tagValuePair[0].item = "Path";
    transactionContext.tagValuePair[0].value =
        g_ui_state.path_and_hash.bip32_path_str;

    transactionContext.tagValuePair[1].item = "Message hash";
    transactionContext.tagValuePair[1].value =
        g_ui_state.path_and_hash.hash_hex;
    ...
    nbgl_useCaseReviewStart(&ICON_APP, "Review message", "", "Reject",
                            sign_message_callback, ux_flow_response_false);
}
...
void ui_display_spend_from_wallet_flow(void) {
    ...
    nbgl_useCaseReviewStart(&ICON_APP, "Spend from\nknown wallet", "", "Cancel",
                            continue_callback, ux_flow_response_false);
}

// Address flow
static void address_display(void) {
    nbgl_useCaseAddressConfirmation(g_ui_state.wallet.address,
                                    status_confirmation_callback);
}
...
void ui_display_canonical_wallet_address_flow(void) {
    ...
    nbgl_useCaseReviewStart(&ICON_APP, "Verify Qtum\naddress", "", "Cancel",
                            address_display, status_cancel);
}

// Warning Flows
void ui_display_warning_external_inputs_flow(void) {
    nbgl_useCaseChoice(&IMPORTANT_CIRCLE_ICON, "Warning",
                       "There are external inputs", "Continue",
                       "Reject if not sure", ux_flow_response);
}

void ui_display_unverified_segwit_inputs_flows(void) {
    nbgl_useCaseChoice(
        &IMPORTANT_CIRCLE_ICON, "Warning",
        "Unverified inputs\nUpdate Ledger Live or\nthird party wallet software",
        "Continue", "Reject if not sure", ux_flow_response);
}

void ui_display_nondefault_sighash_flow(void) {
    nbgl_useCaseChoice(&IMPORTANT_CIRCLE_ICON, "Warning", "Non-default sighash",
                       "Continue", "Reject if not sure", ux_flow_response);
}
...
void ui_display_post_processing_confirm_wallet_spend(bool success) {
    if (success) {
        ...
        nbgl_useCaseStatus("Wallet name rejected", false,
                           ux_flow_response_false);
    }
}
...
void ui_warning_contract_data(void) {
    nbgl_useCaseChoice(
        &IMPORTANT_CIRCLE_ICON, "This message cannot\nbe clear-signed",
        "Enable blind-signing in\nthe settings to sign\nthis transaction.",
        "Exit", "Go to settings", ui_warning_contract_data_choice);
}
