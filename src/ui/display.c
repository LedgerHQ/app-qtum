...
bool ui_display_policy_map_cosigner_pubkey(dispatcher_context_t* context,
                                           ...
                                           uint8_t n_keys, bool is_internal) {
    (void)(n_keys);
...
bool ui_validate_transaction(dispatcher_context_t* context,
    ...
    ui_accept_transaction_flow(is_self_transfer, sign_sender);

    return io_ui_process(context, true);
}

#ifdef HAVE_BAGL
bool ui_post_processing_confirm_wallet_registration(
    dispatcher_context_t* context, bool success) {
    (void)context;
    (void)success;
    return true;
}

bool ui_post_processing_confirm_wallet_spend(dispatcher_context_t* context,
                                             bool success) {
    (void)context;
    (void)success;
    return true;
