...
UX_STEP_NOCB(ux_validate_delegate_to_step, bnnn_paging,
             {
                 .title = "Delegate to",
                 .text = g_ui_state.validate_output.address_or_description,
             });

UX_STEP_NOCB(ux_validate_staker_fee_step, bnnn_paging,
             {
                 .title = "Fee",
                 .text = g_ui_state.validate_output.staker_fee,
             });

UX_STEP_NOCB(ux_confirm_transaction_step, pnn,
...
UX_STEP_CB(ux_accept_and_send_step, pbb, set_ux_flow_response(true),
           {&C_icon_validate_14, "Accept", "and send"});

UX_STEP_CB(ux_sign_sender_step, pbb, set_ux_flow_response(true),
...
UX_STEP_NOCB(ux_message_sign_display_path_step, bnnn_paging,
             {
                 .title = "Path",
...
#else
UX_STEP_CB(ux_warning_contract_data_step, pnn, set_ux_flow_response(false),
           {
               &C_icon_crossmark,
...
UX_FLOW(ux_sign_message_flow, &ux_sign_message_step,
...
UX_FLOW(ux_display_canonical_wallet_address_flow,
        &ux_display_wallet_address_step, &ux_display_approve_step,
...
UX_FLOW(ux_display_spend_from_wallet_flow,
        ...
        &ux_display_wallet_name_step, &ux_display_approve_step,
        &ux_display_reject_step);
...
UX_FLOW(ux_display_output_delegate_flow, &ux_review_step,
        &ux_validate_amount_step, &ux_validate_delegate_to_step,
        &ux_validate_staker_fee_step, &ux_display_approve_step,
        &ux_display_reject_step);
...
// #4 screen: reject button
UX_FLOW(ux_accept_transaction_flow, &ux_confirm_transaction_step,
...
// #4 screen: reject button
UX_FLOW(ux_sign_sender_transaction_flow, &ux_confirm_transaction_step,
        &ux_confirm_transaction_fees_step, &ux_sign_sender_step,
        &ux_display_reject_step);
...
void ui_sign_message_flow(void) { ux_flow_init(0, ux_sign_message_flow, NULL); }
