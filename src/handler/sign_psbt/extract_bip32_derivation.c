...
static void fpt_der_data_len_callback(size_t data_length,
                                      void* callback_state) {
...
static void fpt_der_data_callback(buffer_t* data, void* callback_state) {
    ...
    // on the first call, compute the length the fingerprint + derivation part
    // of the message.
    ...
    if (cs->out_data_length == -1) {
        ...
        if (!is_tap) {
            ...
            if ((!buffer_read_varint(data, &n_hashes)) ||
                (cs->total_data_length <
                 varint_size(n_hashes) + 32 * (int)n_hashes)) {
                PRINTF("Unexpected: initial callback message too short\n");
                cs->result = -1;
                return;
            }

            int out_data_length = cs->total_data_length -
                                  varint_size(n_hashes) - 32 * (int)n_hashes;

            if (out_data_length > 4 * (1 + MAX_BIP32_PATH_STEPS)) {
                PRINTF(
                    "BIP32 derivation longer than supported in psbt "
                    "derivation\n");
                cs->result = -1;
                return;
            }
    ...
    // then, keep exactly the last cs->out_data_length streamed bytes; as they
    // might be streamed across multiple messages, we need to handle it
    // appropriately
    ...
    if (data->size >= (size_t)cs->out_data_length) {
        ...
        // We need to concatenate the new data we are reading with any
        // previously read data. Since we can only read data->size bytes, only
        // the last d = out_data_length - data->size previous bytes are kept;
        // they move from position out_data_length - d + 1 to position 0.
...
int extract_bip32_derivation(dispatcher_context_t* dc, int psbt_key_type,
                             const uint8_t values_root[static 32],
                             uint32_t merkle_tree_size, int index,
                             uint32_t out[static 1 + MAX_BIP32_PATH_STEPS]) {
    fpt_der_callback_data_t callback_state;

    // we could recycle out instead of creating a new array, but we rather keep
    // the code clean, as this is not used in memory-critical parts.
    ...
    int len = call_stream_merkle_leaf_element(
        dc, values_root, merkle_tree_size, index, fpt_der_data_len_callback,
        fpt_der_data_callback, &callback_state);

    if (len < 0 || callback_state.result < 0 ||
        callback_state.out_data_length < 4 ||
