...
static void test_format_sats_amount(void** state) {
    ...
    for (unsigned int i = 0;
         i < sizeof(sats_testcases) / sizeof(sats_testcases[0]); i++) {
        char out[MAX_AMOUNT_LENGTH + 1] = {0};
        format_sats_amount(sats_testcases[i].coin, sats_testcases[i].amount,
                           out);
...
int main() {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_format_sats_amount)};
